...
impl SecretKey {
    ...
    pub fn eval<D: Digest<OutputSize = U64>>(&self, seed: &[u8]) -> VerifiableRandomness {
        let uʹ = ristretto_point_from_hash::<D>(seed);
        let u = &self.sk * &uʹ;
        ...
        let π = LdeiProof::prove::<D>(&witness, &stmt).expect("stmt must be valid");

        let y = D::new().chain(seed).chain_point(&u).result();
...
impl VerifiableRandomness {
    pub fn verify<D: Digest<OutputSize = U64>>(
        &self,
        pk: &PublicKey,
        seed: &[u8],
    ) -> Result<(), ProofError> {
        ...
        self.proof.π.verify::<D>(&stmt)?;

        let y = D::new().chain(seed).chain_point(&self.proof.u).result();
        if y != self.y {
...
fn ristretto_point_from_hash<D: Digest<OutputSize = U64>>(bytes: &[u8]) -> Point<Ristretto> {
    let mut uniform_bytes = [0; 64];
    uniform_bytes.copy_from_slice(&D::new().chain(bytes).result());
